...
- **v2.2 — ✅ SHIPPED** (Pillar D). Every draft card has an **Edit** button → a modal pre-filled with the draft; saving updates the post (Approve publishes the edited text) and `distill_edit_memory` diffs original→edited into durable style rules stored as `VoiceMemory` per `(slack_user_id, platform)` (a separate table, so seed users accrue memory too). Those rules ride on every future draft as an extra cached system block (`VoiceContext.memory`, capped by `VOICE_MEMORY_LIMIT`). `/voice` lists the learned rules per platform with a **Forget** button (`delete_voice_memory`). Only edits are mined (not approve/regenerate) to keep the signal intentional; the edit fixes the current post, memory steers the next. *(Closes the loop: it gets better the more you use it.)*
